...
void write_dump(std::string_view const bytes) {
  auto out = std::ofstream{
      "dump_rt/test-https___example_test_trip_updates",
...
double event_total(std::string const& metrics, std::string_view const event) {
  auto total = 0.0;
  auto const event_label =
      std::string{"event=\""}.append(event).append("\"");
  ...
  while (line_begin != std::string::npos && line_begin < metrics.size()) {
    ...
    auto const line = std::string_view{metrics}.substr(
        line_begin, line_end == std::string::npos
                        ? std::string::npos
                        : line_end - line_begin);
...
TEST(motis_rt_update, fresh_last_good_survives_bad_cycle_and_expires) {
      ...
      .timetable_ = {config::timetable{
          ...
          .datasets_ = {
              {"test",
               {.path_ = gtfs,
                .rt_ = {{{.url_ =
                               "https://example.test/trip_updates",
                           .last_good_ttl_ = 5U}}}}}}}}};
  import(c, "data");
  auto d = data{"data", c};
  ...
  old_payload.mutable_header()->set_timestamp(
  ...
  ASSERT_TRUE(
      fs::remove("dump_rt/test-https___example_test_trip_updates"));
  ioc.restart();
  ioc.run_for(1100ms);

  auto const metrics = prometheus::TextSerializer{}.Serialize(
      d.metrics_->registry_.Collect());
  ...
  EXPECT_THAT(metrics,
              HasSubstr("nigiri_gtfsrt_source_cache_age_seconds{"));
