...
// Test case with OSM + GBFS + Geocoding
...
template <>
test_case_params const import_test_case<test_case::FFM_routing>() {
  auto const c = config{
  ...
  return import_test_case(std::move(c), "test/test_case/ffm_routing_data");
}
