          ...
          [&, cache = street_routing_cache_t{}](auto&& j) mutable {
            ...
            if (response.legs_.front().mode_ == api::ModeEnum::RIDE_SHARING) {
              for (auto const [i, a] :
                ...
                if (a.time_at_start_ == response.legs_.front().startTime_ && // not looking at time_at_stop_ because we would again need to take into account the 5 min shift...
            ...
            if (response.legs_.back().mode_ == api::ModeEnum::RIDE_SHARING) {
              for (auto const [i, a] :
                ...
                if (a.time_at_start_ == response.legs_.back().endTime_ && // not looking at time_at_stop_ because we would again need to take into account the 5 min shift...
                    r_.tags_->id(*tt_, a.stop_) ==
