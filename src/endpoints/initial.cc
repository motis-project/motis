...
api::initial_response initial::operator()(boost::urls::url_view const&) const {
  ...
  auto const onetoall_max_travel_minutes = config_.limits_->onetoall_max_travel_minutes_;
  auto const street_routing_max_direct_seconds = config_.limits_->street_routing_max_direct_seconds_;
  auto const street_routing_max_prepost_transit_seconds = config_.limits_->street_routing_max_prepost_transit_seconds_;
  auto const osr_footpath = config_.osr_footpath_;
  auto const has_elevation_data_dir = config_.get_street_routing().has_value() && config_.get_street_routing()->elevation_data_dir_.has_value();
  ...
  return {.lat_ = center.lat_,
          ...
          .maxPrePostTransitTimeLimit_ = static_cast<double>(street_routing_max_prepost_transit_seconds),
          .maxDirectTimeLimit_ = static_cast<double>(street_routing_max_direct_seconds)
        };
}
