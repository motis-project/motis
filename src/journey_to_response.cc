              ...
              auto const write_run_leg = [&](auto,
                ...
                auto& leg = itinerary.legs_.emplace_back(api::Leg{
                    .mode_ = to_mode(enter_stop.get_clasz(n::event_type::kDep),
                                     api_version),
                    .from_ = patch(to_place(enter_stop, n::event_type::kDep), api_version),
                    .to_ = patch(to_place(exit_stop, n::event_type::kArr), api_version),
                    .duration_ =
                    ...
                    .tripFrom_ =
                        [&]() {
                          auto const first = exit_stop.get_first_trip_stop(
                              n::event_type::kArr);
                          auto p = patch(to_place(first, n::event_type::kDep), api_version);
                    ...
                    .tripTo_ =
                        [&]() {
                          auto const last = enter_stop.get_last_trip_stop(
                              n::event_type::kDep);
                          auto p = patch(to_place(last, n::event_type::kArr), api_version);
                ...
                for (auto i = first; i < last; ++i) {
                  ...
                  auto& p = leg.intermediateStops_->emplace_back(
                      patch(to_place(stop, n::event_type::kDep), api_version));
