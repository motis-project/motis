...
/**
 * Computes optimal connections from one place to another,
 * with the option to provide client-computed offsets
 * for the first and/or last mile.
 *
 * All routing parameters are passed as query parameters
 * (same as the `plan` GET endpoint - see there for their descriptions).
 * The request body carries only the offset lists.
 *
 */
export const planPost = <ThrowOnError extends boolean = false>(options: Options<PlanPostData, ThrowOnError>) => {
    return (options?.client ?? client).post<PlanPostResponse, PlanPostError, ThrowOnError>({
        ...options,
        url: '/api/v6/plan'
    });
};
