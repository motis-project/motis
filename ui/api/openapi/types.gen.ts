...
/**
 * A vehicle class that the `minimizeWithout` routing criterion prefers to
 * avoid. Supported: `AIR` (flights) and `COACH`.
 *
 */
export type VehicleClassFilter = 'AIR' | 'COACH';
...
export type PlanData = {
    query: {
        ...
        maxTravelTime?: number;
        /**
         * Add "number of vehicle-class switches between consecutive trips" as an
         * extra Pareto criterion (`BUS -> SUBWAY` counts, `SUBWAY -> SUBWAY`
         * does not), rewarding journeys that stay within one mode.
         *
         * Setting this (or any other `minimize*` parameter) switches routing to
         * the multicriteria engine, overriding `algorithm` if it was `PONG`.
         *
         */
        minimizeModeSwitches?: boolean;
        /**
         * Add "minutes not on transit" (pre-/post-transit legs and transfer
         * footpaths) as an extra Pareto criterion, so the result also keeps
         * journeys that trade a little arrival time for less walking.
         *
         * Setting this (or any other `minimize*` parameter) switches routing to
         * the multicriteria engine, overriding `algorithm` if it was `PONG`.
         *
        ...
        /**
         * Vehicle classes to add as an "avoid" Pareto criterion: the result
         * keeps both the fastest journey and the best one that does not use any
         * of these classes, instead of filtering them out.
         *
         * Supported: `AIR` (flights) and `COACH`. Passing both avoids either -
         * the result adds the best journey that uses neither class, not one
         * trade-off per class.
         *
         * Setting this (or any other `minimize*` parameter) switches routing to
         * the multicriteria engine, overriding `algorithm` if it was `PONG`.
         *
         */
        minimizeWithout?: Array<VehicleClassFilter>;
        /**
         * Optional. Default is 0 minutes.
         *
         * Minimum transfer time for each transfer in minutes.
