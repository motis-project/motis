...
nigiri::routing::td_offsets_t get_td_offsets(
  ...
  utl::equal_ranges_linear(
      ...
      [&](auto&& from_it, auto&& to_it) {
        ...
        for (auto const& r : nigiri::it_range{from_it, to_it}) {
          ...
          auto i = std::lower_bound(begin(td_offsets.at(r.stop_)), end(td_offsets.at(r.stop_)), tdo, [](auto const& a, auto const& b) {
            return a.valid_from_ < b.valid_from_;
          });

          if (i == end(td_offsets.at(r.stop_)) || tdo.valid_from_ < i->valid_from_) {
            i = td_offsets.at(r.stop_).insert(i,tdo);
          } else if (tdo.duration_ < i->duration_) {
              *i = tdo;
            }

          if (i + 1 == end(td_offsets.at(r.stop_)) || (i+1)->valid_from_ != tdo.valid_from_ + 1min) {
            td_offsets.at(r.stop_).insert(i+1, {.valid_from_ = tdo.valid_from_ + 1min, .duration_ = nigiri::footpath::kMaxDuration, .transport_mode_id_ = mode});
          }
        }
      });
  ...
  for (auto & [l, tdos] : td_offsets) {
    for (auto i = begin(tdos); i != end(tdos);) {
      if (begin(tdos) < i && (i-1)->duration_ == i->duration_ && (i-1)->transport_mode_id_ == i->transport_mode_id_) {
        i = tdos.erase(i);
      } else {
        ++i;
      }
