...
./motis print fail/$1_intermodal-responses-fwd-{routing,nigiri}.json

rm -rf input/$1
mkdir input/$1
ln -s `pwd`/input/osm.pbf input/$1/osm.pbf

./motis xtract input/hrd input/$1 fail/$1_intermodal-responses-fwd-{routing,nigiri}.json
...
./motis \
  ...
  --batch_input_file=fail/$1_intermodal-queries-fwd-routing.json \
...
./motis \
  ...
  --import.paths schedule:input/$1 osm:input/osm.pbf \
  --import.data_dir data_$1 \
  --batch_input_file=fail/$1_intermodal-queries-fwd-nigiri.json \
...
${DIR}/motis intermodal_compare \
    --queries \
        ${DIR}/fail/$1_intermodal-queries-fwd-routing.json \
        ${DIR}/fail/$1_intermodal-queries-fwd-nigiri.json \
    --input \
