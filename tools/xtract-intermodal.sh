...
./motis print fail/$1_responses_{routing,nigiri}.json

rm -rf input-$1
mkdir input-$1
ln -s `pwd`/input/osm.pbf input-$1/osm.pbf

./motis xtract input/hrd input-$1 fail/$1_responses_{routing,nigiri}.json
...
./motis \
  ...
  --batch_input_file=fail/$1_queries_routing.json \
...
./motis \
  ...
  --import.paths schedule:input-$1 osm:input/osm.pbf \
  --import.data_dir data_$1 \
  --batch_input_file=fail/$1_queries_nigiri.json \
...
${DIR}/motis intermodal_compare \
    --fail "" \
    --queries \
        ${DIR}/fail/$1_queries_routing.json \
        ${DIR}/fail/$1_queries_nigiri.json \
    --input \
