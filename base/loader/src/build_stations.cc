...
struct stations_builder {
  ...
  void add_station(uint32_t const source_schedule, Station const* fbs_station,
    ...
    s->eva_nr_ = std::string{sched_.prefixes_[source_schedule]} +
                 fbs_station->id()->str();
    s->transfer_time_ = static_cast<duration>(std::max(1, fbs_station->interchange_time()));
    s->platform_transfer_time_ = static_cast<duration>(fbs_station->platform_interchange_time());
