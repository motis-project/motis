...
full_trip_id graph_builder::get_full_trip_id(Service const* s, int day,
  ...
  auto const dep_time = get_adjusted_event_time(
      ...
      dep_provider == nullptr ? nullptr : c_str(dep_provider->timezone_name()));
...
void graph_builder::add_route_services(
  ...
  auto const has_traffic_within_timespan =
      [&](Service const* const s, bitfield const& traffic_days,
          day_idx_t const start_idx, day_idx_t const end_idx) {
        for (auto day_idx = start_idx; day_idx < end_idx; ++day_idx) {
          if (traffic_days.test(day_idx)) {
            return true;
          }
  ...
  bool graph_builder::has_duplicate(
      Service const* service, mcd::vector<light_connection> const& lcons) {
  ...
  bool graph_builder::are_duplicates(
      Service const* service_a, mcd::vector<light_connection> const& lcons_a,
      trip const* trp_b) {
  ...
  light_connection graph_builder::section_to_connection(
    ...
    assert(
        std::all_of(begin(services), end(services), [&](participant const& s) {
    ...
    sched_.last_event_schedule_time_ =
        std::max(sched_.last_event_schedule_time_,
                 motis_to_unixtime(sched_, arr_motis_time) -
                     SCHEDULE_OFFSET_MINUTES * 60);
  ...
  mcd::unique_ptr<route> graph_builder::create_route(
      Route const* r, route_lcs const& lcons, unsigned route_index) {
  ...
  route_section graph_builder::add_route_section(
    ...
    if (to_route_node != nullptr) {
      ...
      section.to_route_node_ = build_route_node(
          ...
          sched_.stations_[to_station_node->id_]->transfer_time_, to_in_allowed,
          to_out_allowed);
    }

    section.outgoing_route_edge_index_ =
        section.from_route_node_->edges_.size();
  ...
  schedule_ptr build_graph(std::vector<Schedule const*> const& fbs_schedules,
    ...
    for (auto const& [i, fbs_schedule] : utl::enumerate(fbs_schedules)) {
      ...
      if (opt.apply_rules_) {
        ...
        progress_tracker
            ->status(fmt::format("Rule Services {}", dataset_prefix))
    ...
    if (opt.expand_trips_) {
      LOG(info) << sched->expanded_trips_.index_size() - 1
                << " expanded routes";
