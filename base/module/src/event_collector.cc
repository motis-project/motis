...
event_collector::event_collector(std::string data_dir, std::string module_name,
  ...
  progress_tracker_.msg("WAITING").show_progress(false);
}
      ...
      [&, matcher_it = matcher_it.first, name,
       self = shared_from_this()](msg_ptr const& msg) -> msg_ptr {
        ...
        if (!waiting_for_.empty()) {
          progress_tracker_.msg(fmt::format("WAITING: {}", waiting_for_));
        ...
        try {
          executed_ = true;
          op_(dependencies_);
          progress_tracker_.msg("FINISHED").show_progress(false);
        } catch (std::exception const& e) {
          progress_tracker_.msg(fmt::format("ERROR: {}", e.what()))
              .show_progress(false);
        } catch (...) {
          progress_tracker_.msg("ERROR: UNKNOWN EXCEPTION")
              .show_progress(false);
        }
