...
std::time_t external_schedule_begin(schedule const&);

std::time_t external_schedule_end(schedule const&);

void verify_timestamp(schedule const&, time_t);

void verify_external_timestamp(schedule const&, time_t);

std::time_t motis_to_unixtime(schedule const&, time);

time unix_to_motistime(schedule const&, std::time_t);

time motis_time(int hhmm, int day_idx = 0,
                int timezone_offset = DEFAULT_TIMEZONE_OFFSET);

std::time_t unix_time(schedule const&, int hhmm, int day_idx = 0,
                      int timezone_offset = DEFAULT_TIMEZONE_OFFSET);
