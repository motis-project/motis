...
std::uint16_t guess_trip_capacity(schedule const& sched, rsl_data& data,
                                  trip const* trp) {
...
std::set<passenger_group*> collect_passenger_groups(trip_data& td) {
  ...
  for (auto const& te : td.edges_) {
    for (auto& psi : te->rsl_connection_info_.section_infos_) {
      if (psi.valid_) {
...
void apply_reroute(rsl_data& data, schedule const& sched, trip const* trp,
