...
void handle_rt_update(paxmon_data& data, schedule const& sched,
  ...
  for (auto const& u : *update->updates()) {
    switch (u->content_type()) {
      case Content_RtDelayUpdate: {
        ...
        update_event_times(sched, data.graph_, du, updated_interchange_edges,
                           system_stats);
  ...
  check_broken_interchanges(data, updated_interchange_edges, system_stats,
...
std::vector<msg_ptr> update_affected_groups(paxmon_data& data,
  ...
  auto const affected_passenger_count = std::accumulate(
      begin(data.groups_affected_by_last_update_),
      end(data.groups_affected_by_last_update_), 0ULL,
      [](auto const sum, auto const& pg) { return sum + pg->passengers_; });

  tick_stats.affected_groups_ = data.groups_affected_by_last_update_.size();
  tick_stats.affected_passengers_ = affected_passenger_count;

  data.trips_affected_by_last_update_.clear();
  ...
  motis_parallel_for(data.groups_affected_by_last_update_, [&](auto const& pg) {
    MOTIS_START_TIMING(reachability);
    auto const reachability =
        get_reachability(data, pg->compact_planned_journey_);
    ...
    auto const event_type =
        get_monitoring_event_type(pg, reachability, arrival_delay_threshold);
    ...
    update_load(pg, reachability, localization, data.graph_);
