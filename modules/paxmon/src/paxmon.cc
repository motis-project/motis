...
void paxmon::init(motis::module::registry& reg) {
  ...
  reg.register_op(
      ...
      [&](msg_ptr const& msg) -> msg_ptr { return get_interchanges(msg); });
...
msg_ptr paxmon::get_interchanges(msg_ptr const& msg) {
  ...
  auto const add_station = [&](unsigned const station_idx) {
    ...
    for (auto const& ei : uv.interchanges_at_station_.at(station_idx)) {
      ...
      if (!ic_edge->is_valid(uv) || (!include_event(ic_edge->from(uv)) &&
