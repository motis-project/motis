...
std::vector<std::pair<service_info, unsigned>> get_service_infos(
  ...
  for (auto const& section : motis::access::sections(trp)) {
    ...
    for (auto ci = fc.con_info_; ci != nullptr; ci = ci->merged_with_) {
      auto const si = get_service_info(sched, ci, fc.clasz_);
      ++si_counts[si];
    }
  }
  if (si_counts.empty() && trp->original_first_connection_info_ != nullptr) {
    si_counts[get_service_info(sched, trp->original_first_connection_info_,
                               trp->original_first_clasz_)] = 1;
  }
