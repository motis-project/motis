...
namespace motis::csa::price_regional {

using price_t = uint16_t;

constexpr price_t MINUTE_PRICE = 8;
constexpr price_t MAX_REGIONAL_PRICE = 4200;
...
inline price_t add_price(price_t base, T additional,
                         uint16_t limit = std::numeric_limits<price_t>::max()) {
...
struct price_data {
  price_t total_{INVALID_PRICE};
  price_t regional_{0};
  price_t other_{0};

  inline void update_total() { total_ = add_price(regional_, other_); }

  inline bool valid() const { return total_ != INVALID_PRICE; }

  inline bool operator<(price_data const& o) const { return total_ < o.total_; }
  inline bool operator<=(price_data const& o) const {
    return total_ <= o.total_;
  }
  inline bool operator>(price_data const& o) const { return total_ > o.total_; }
  inline bool operator>=(price_data const& o) const {
    return total_ >= o.total_;
...
template <typename T>
inline price_data add_price(price_data base, T additional) {
  auto sum = base;
  sum.other_ = add_price(sum.other_, additional);
  sum.update_total();
  return sum;
}

inline price_data const& min_price(price_data const& a, price_data const& b) {
  if (a.total_ < b.total_) {
    return a;
  } else if (b.total_ < a.total_) {
    return b;
  } else {
    return a.regional_ >= b.regional_ ? a : b;
  }
}
...
struct station_arrival_info {
  ...
  station_arrival_info(time arrival_time, price_data price)
  ...
  inline bool dominates_price(station_arrival_info const& other) const {
    ...
    auto const this_regional_remaining = MAX_REGIONAL_PRICE - price_.regional_;
    auto const other_regional_remaining =
        MAX_REGIONAL_PRICE - other.price_.regional_;

    uint32_t const this_price =
        (time_ > other.time_) == fwd
            ? price_.total_ + other_regional_remaining
            : price_.total_ + other_regional_remaining + min_wage_diff;
    uint32_t const other_price =
        (time_ > other.time_) == fwd
            ? other.price_.total_ + this_regional_remaining + min_wage_diff
            : other.price_.total_ + this_regional_remaining;
...
struct csa_search {
  ...
  void add_start(csa_station const& station, time initial_duration,
    ...
    stats_.start_count_++;
    std::array<price_data, MAX_TRANSFERS + 1> arrival_prices{};
    arrival_prices[0].other_ = initial_price + initial_duration * MINUTE_PRICE;
    arrival_prices[0].update_total();
  ...
  void search() {
    ...
    for (auto it = first_connection; it != end(connections); ++it) {
      ...
      std::array<price_data, MAX_TRANSFERS + 1> trip_reachable_prices;
      ...
      for (auto transfers = 0; transfers < MAX_TRANSFERS; ++transfers) {
        ...
        //        std::cout << "  transfers=" << transfers
        if (!via_trip_price.valid()) {
          stats_.reachable_via_trip_++;
        }
        if (!via_station_price.valid()) {
          stats_.reachable_via_station_++;
        }

        if (via_trip_price.valid() || via_station_price.valid()) {
          auto const departure_price =
              min_price(via_trip_price, via_station_price);
          auto const via_station =
              via_station_price < via_trip_price ||
              (via_station_price.total_ == via_trip_price.total_ &&
               via_station_price.regional_ > via_trip_price.regional_);
          if (via_station) {
          ...
          arrival_prices[transfers + 1] = departure_price;  // NOLINT
          add_connection_price(arrival_prices[transfers + 1], con,  // NOLINT
  ...
  inline std::array<price_data, MAX_TRANSFERS + 1>& price_via_trip(
      csa_connection const& con) {
    auto& tr = trip_reachable_[con.trip_];
    if (tr.empty()) {
      tr.resize(tt_.trip_to_connections_[con.trip_].size());
      stats_.trip_price_init_++;
    }
    return tr[con.trip_con_idx_];
  }

  inline price_data price_via_station(csa_connection const& con,
                                      int transfers) {
  ...
  inline void add_connection_price(price_data& price, csa_connection const& con,
                                   price_t min_price) {
    if (con.clasz_ >= MOTIS_RE) {
      price.regional_ =
          add_price(price.regional_, con.price_, MAX_REGIONAL_PRICE);
      price.other_ = add_price(price.other_, min_price);
    } else {
      price.other_ =
          add_price(price.other_, static_cast<uint32_t>(con.price_) +
                                      static_cast<uint32_t>(min_price));
    }
    price.update_total();
  }
  ...
  inline void update_trip_reachable(
      csa_connection const& con,
      std::array<price_data, MAX_TRANSFERS + 1> const& initial_prices) {
    auto& tr = trip_reachable_[con.trip_];
    ...
    std::array<price_data, MAX_TRANSFERS + 1> price{};
    for (auto transfers = 0; transfers < MAX_TRANSFERS + 1; ++transfers) {
      auto const initial_price = initial_prices[transfers];  // NOLINT
      if (initial_price.valid()) {
        assert(tr[con.trip_con_idx_][transfers] > initial_price ||  // NOLINT
               (tr[con.trip_con_idx_][transfers].total_ ==  // NOLINT
                    initial_price.total_ &&
                tr[con.trip_con_idx_][transfers].regional_ <  // NOLINT
                    initial_price.regional_));
    ...
    if (Dir == search_dir::FWD) {
      for (auto con_idx = con.trip_con_idx_ + 1UL; con_idx < trip_cons.size();
        ...
        auto const min_price =
            (cur_con->departure_ - prev_con->departure_) * MINUTE_PRICE;
        for (auto transfers = 0; transfers < MAX_TRANSFERS + 1; ++transfers) {
          if (!update[transfers]) {  // NOLINT
            continue;
          }
          add_connection_price(price[transfers], *prev_con,  // NOLINT
                               min_price);
          if (price[transfers] > tr[con_idx][transfers] ||  // NOLINT
              (price[transfers].total_ ==  // NOLINT
                   tr[con_idx][transfers].total_ &&
               price[transfers].regional_ <=  // NOLINT
                   tr[con_idx][transfers].regional_)) {
            return;
          }
  ...
  void expand_footpaths(
      ...
      std::array<price_data, MAX_TRANSFERS + 1> const& arrival_prices) {
    if (Dir == search_dir::FWD) {
      for (auto const& fp : station.footpaths_) {
        ...
        for (auto transfers = 0; transfers <= MAX_TRANSFERS; ++transfers) {
          if (!arrival_prices[transfers].valid()) {  // NOLINT
            continue;
          }
  ...
  std::vector<csa_journey> get_results(csa_station const& station,
    ...
    for (auto transfers = 0; transfers <= MAX_TRANSFERS; ++transfers) {
      for (auto const& sai : station_arrival[transfers]) {  // NOLINT
        ...
        if (!dominated(dur, sai.price_.total_)) {
          journeys.emplace_back(Dir, start_time_, sai.time_, transfers,
                                &station, sai.price_.total_);
        }
      }
    }
    return journeys;
  ...
  journey_pointer get_journey_pointer(csa_station const& station,
    ...
    if (Dir == search_dir::FWD) {
      for (auto const& arrival : station_arrival) {
        ...
        if (arrival.price_.total_ != arrival_price) {
          continue;
        }
        for (auto const& fp : station.incoming_footpaths_) {
  ...
  std::vector<csa_connection const*> get_exit_candidates(
    ...
    std::copy_if(
        begin(arrival_station.incoming_connections_),
        end(arrival_station.incoming_connections_),
        std::back_inserter(candidates), [&](csa_connection const* con) {
          auto const& tr = trip_reachable_[con->trip_];
          return con->arrival_ == arrival_time && !tr.empty() &&
                 tr[con->trip_con_idx_][transfers - 1].total_ ==  // NOLINT
  ...
  csa_connection const* get_enter_connection(csa_connection const* exit_con,
    ...
    for (auto i = exit_con->trip_con_idx_;; --i) {
      if (tr[i][transfers - 1].total_ == departure_price) {  // NOLINT
        enter_con_idx = i;
        if (i == 0) {
          break;
        }
  ...
  std::vector<csa_connection const*> get_enter_candidates(
    ...
    std::copy_if(
        begin(departure_station.outgoing_connections_),
  ...
  csa_connection const* get_exit_connection(csa_connection const* enter_con,
    ...
    for (auto i = enter_con->trip_con_idx_;; ++i) {
      if (tr[i][transfers - 1].total_ == arrival_price) {  // NOLINT
        exit_con_idx = i;
        if (i == cons.size() - 1) {
          break;
        }
  ...
  std::vector<std::array<std::vector<station_arrival_info>, MAX_TRANSFERS + 1>>
      arrival_;
  std::vector<std::vector<std::array<price_data, MAX_TRANSFERS + 1>>>
      trip_reachable_;
