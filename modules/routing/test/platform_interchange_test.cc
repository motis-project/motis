...
struct platform_interchange_test_base : public motis_instance_test {
  explicit platform_interchange_test_base(loader::loader_options const& opt)
      : motis::test::motis_instance_test(opt, {"routing"}, {}) {}
...
struct platform_interchange_test : public platform_interchange_test_base {
  platform_interchange_test() : platform_interchange_test_base(dataset_opt) {}
...
TEST_F(platform_interchange_test, same_platform_bwd) {
  auto res = call(bwd_routing_request());
  auto journeys = message_to_journeys(motis_content(RoutingResponse, res));
  check_result(journeys);
}

struct platform_interchange_no_platforms_test
    : public platform_interchange_test_base {
  platform_interchange_no_platforms_test()
