...
lookup_station nigiri_station_lookup::get(std::size_t const idx) const {
  ...
  return {
      .tag_ = tags_.get_tag(tt_.locations_.src_[l]),
      .id_ = tt_.locations_.ids_[l].view(),
      .name_ = (p == n::location_idx_t::invalid() ? tt_.locations_.names_[l]
                                                  : tt_.locations_.names_[p])
                   .view(),
      .pos_ = tt_.locations_.coordinates_[l]};
}
