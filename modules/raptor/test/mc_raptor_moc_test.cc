...
TEST_F(mc_cpu_raptor_tests, tc1_force_trip_change_but_keep_route) {
  ...
  auto const journeys = execute_mc_cpu_raptor<MaxOccupancy>(
...
//              stop at B
TEST_F(mc_cpu_raptor_tests, tc5_force_exit_and_exter_trip_with_loop) {
  auto& sched = *this->sched_;
  auto& meta_info = this->rp_meta_info_;
  auto& tt = this->rp_tt_;
...
TEST_F(mc_cpu_raptor_tests, tc6_force_later_dep_station_on_same_trip) {
  auto& sched = *this->sched_;
  auto& meta_info = this->rp_meta_info_;
  auto& tt = this->rp_tt_;
...
TEST_F(mc_cpu_raptor_tests, tc7_allow_multiple_conns_on_single_trip) {
  auto& sched = *this->sched_;
  auto& meta_info = this->rp_meta_info_;
  auto& tt = this->rp_tt_;
