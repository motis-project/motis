...
void invoke_hybrid_raptor(d_query& dq) {
  ...
  auto const& proc_stream = device.streams_[0];
  auto const& transfer_stream = device.streams_[1];

  void* init_args[] = {(void*)&dq};

  launch_kernel(init_arrivals_kernel, init_args, *dq.device_, proc_stream);
  ...
  for (int k = 1; k < max_raptor_round; ++k) {
    ...
    launch_kernel(update_routes_kernel, kernel_args, device, proc_stream);
    cuda_sync_stream(proc_stream);
