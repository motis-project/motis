...
// void write_to_fbs(mcd::vector<station_seq> const& sequences,
...
//         reinterpret_cast<void const*>(fbb.GetBufferPointer()), 1, size,
//         file);
//     utl::verify(w_buf == size, "buf write failed");
//   }

//   uint32_t terminal = 0;
//   auto const w_terminal = std::fwrite(reinterpret_cast<void
//   const*>(&terminal),
//                                       sizeof(uint32_t), 1, file);
...
// mcd::vector<station_seq> read_from_fbs(std::string const& fname) {
...
//   mcd::vector<station_seq> result(vector_offset);
//   utl::parallel_for(batches, [&](auto const& batch) {
//     auto cache = GetResolvedStationSeqCache(mmap.ptr() +
//     batch.buffer_offset_); for (auto i = 0u; i < cache->sequences()->size();
//     ++i) {
...
//             std::begin(*seg->osm_node_ids()),
//             std::end(*seg->osm_node_ids()));
//         path.verify_path();
//         return path;
//       });
...
//       seq.distance_ = cached_seq->distance();
