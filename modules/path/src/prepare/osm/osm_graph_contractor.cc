...
osm_graph_contractor::osm_graph_contractor(osm_graph const& graph)
  ...
  for (auto const& link : graph_.node_station_links_) {  // todo maybe parallel?
...
void osm_graph_contractor::contract() {
  ...
  for (auto const& [offset, node_count] : graph_.component_offsets_) {
    // auto const component_worker_count =
    //     std::min(node_count / 1000, concurrency);
    // for (auto i = 0ULL; i < num_tasks; ++i) {
    // }

    work.push_back({offset, node_count});
  }
...
std::vector<size_t> osm_graph_contractor::get_neighbors(
    contract_node const& curr) {
...
void osm_graph_contractor::process_node(contract_node& curr) {
